...
async function main(
  ...
  const fileNameToCheck = `./week${currentCalculation}/${vaultName}-rewards-${startBlock}-${endBlock}.json`;
  ...
  if (!skip) {
    ...
    const fileName = `./week${currentCalculation}/${vaultName}-QI-rewards-${startBlock}-${endBlock}.json`;
